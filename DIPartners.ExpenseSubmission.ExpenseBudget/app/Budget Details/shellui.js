...
function OnNewShellUI(shellUI) {
    ...
    console.initialize(shellUI, "Budget");
...
function OnNewNormalShellFrame(shellFrame) {
    ...
    function OnStarted() {
        myTab = shellFrame.RightPane.AddTab("Budget", "Budget", "_last");
    }
    ...
    function OnSelectionChanged(selectedItems) {
        ...
        if (IsExactlyOneDocumentSelected(selectedItems)){
			ShowDashboard(selectedItems.ObjectVersions[0], selectedItems);	
		}
        else
            HideDashboard();
    }
    ...
    function IsExactlyOneDocumentSelected(selectedItems) {
        ...
        if (selectedItem.Class != shellFrame.ShellUI.Vault.ClassOperations.GetObjectClassIDByAlias("vClass.ExpenseSubmission"))
            return false;
    ...
    function ShowDashboard(objectVersion, selectedItems) {

        // Show dashboard content in right pane tab. Ensure that the tab is visible.
        myTab.ShowDashboard("Budget", { ObjectVersion: objectVersion, SelectedItems: selectedItems });
		//myTab.ShowDashboard("APInvoice", { ObjectVersion: objectVersion, PrevSelectedItems: prevSelectedItems, SelectedItems: selectedItems });
        myTab.Visible = true;
    }
